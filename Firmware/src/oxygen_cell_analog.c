...
static int analog_adc_read(struct analog_cell_state *cell)
{
	cell->adc_seq.buffer = &cell->adc_sample_buf;
	cell->adc_seq.buffer_size = sizeof(cell->adc_sample_buf);

	int ret = adc_read(cell->adc_dev, &cell->adc_seq);
...
static void analog_publish(struct analog_cell_state *cell)
{
	...
	float cal_ppo2 = analog_calculate_ppo2(cell->last_counts,
					       cell->cal_coeff);
	if (cal_ppo2 > 255.0f) {
		cell->status = CELL_FAIL;
	...
	OxygenCellMsg_t msg = {
		...
		.ppo2 = ppo2,
		.precision_ppo2 = (double)cal_ppo2 / 100.0,
...
static int analog_cell_init_adc(struct analog_cell_state *cell)
{
...
static void analog_cell_thread(void *p1, void *p2, void *p3)
{
	...
	if (analog_cell_init_adc(cell) != 0) {
		cell->status = CELL_FAIL;
		return;
	}

	while (true) {
		if (analog_adc_read(cell) == 0) {
			analog_publish(cell);
		}
