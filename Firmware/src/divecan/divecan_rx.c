			     ...
			     void *user_data)
{
	...
	if (k_msgq_put(&can_rx_msgq, &msg, K_NO_WAIT) != 0) {
...
static void divecan_rx_thread(void *p1, void *p2, void *p3)
{
	...
	int ret = divecan_tx_init(can_dev);
	if (ret != 0) {
	...
	int ext_filter_id = can_add_rx_filter(can_dev, can_rx_callback, NULL,
					      &ext_filter);
	if (ext_filter_id < 0) {
	...
	while (true) {
		...
		int rx_ret = k_msgq_get(&can_rx_msgq, &message,
...
{
	CalResponse_t resp;
	if (zbus_chan_read(chan, &resp, K_NO_WAIT) != 0) {
		return;
	}

	DiveCANCalResponse_t divecan_result;
	if (resp.result == CAL_RESULT_OK) {
		divecan_result = DIVECAN_CAL_RESULT_OK;
	} else if (resp.result == CAL_RESULT_REJECTED) {
		divecan_result = DIVECAN_CAL_FAIL_REJECTED;
	} else {
		divecan_result = DIVECAN_CAL_FAIL_GEN;
	}
	...
	txCalResponse(device_spec.type, divecan_result,
		      resp.cell_mv[0], resp.cell_mv[1], resp.cell_mv[2],
...
static void RespPing(const DiveCANMessage_t *message)
{
	...
	if ((sender == DIVECAN_CONTROLLER) || (sender == DIVECAN_MONITOR)) {
		...
		float supplyVoltage = power_get_battery_voltage(POWER_DEVICE);
		...
		BatteryV_t batteryV = (BatteryV_t)(supplyVoltage *
						   (float)BATTERY_FLOAT_TO_INT);
...
static void RespCal(const DiveCANMessage_t *message)
{
	...
	uint16_t pressure = (uint16_t)(
		((uint16_t)message->data[1] << DIVECAN_BYTE_WIDTH) |
		message->data[2]);
...
{
	uint16_t pressure = (uint16_t)(
		((uint16_t)message->data[2] << DIVECAN_BYTE_WIDTH) |
		message->data[3]);
