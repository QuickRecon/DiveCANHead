...
{
	return atomic_get(&cal_running) != 0;
}
...
static int cal_save_coefficient(uint8_t cell_num, CalCoeff_t coeff)
{
	char key[16];
...
static int cal_load_coefficient(uint8_t cell_num, CalCoeff_t *coeff)
{
	char key[16];
 ...
 *
 * Returns the new coefficient on success, or negative value on error.
				  ...
				  ShortMillivolts_t *mv_out)
{
	...
	*mv_out = 0;
	...
	{
		/* Read current ADC counts from cell 1 state.
		 * For analog cells, we need the raw ADC counts to compute
		 * the calibration coefficient. Currently we read from the
		 * zbus channel's last published millivolts instead, since
		 * the cell state is private to its thread. */
		OxygenCellMsg_t cell_data;

		if (zbus_chan_read(&chan_cell_1, &cell_data, K_MSEC(100)) != 0) {
		...
		int16_t approx_counts = (int16_t)(cell_data.millivolts /
						  COUNTS_TO_MILLIS);

		new_coeff = analog_cal_coefficient(approx_counts, target_ppo2);
...
#elif defined(CONFIG_CELL_1_TYPE_DIVEO2)
	{
		...
		int32_t approx_sample = (int32_t)(cell_data.precision_ppo2 *
						  (double)DIVEO2_CAL_DEFAULT);

		new_coeff = diveo2_cal_coefficient(approx_sample, target_ppo2);
	}
#elif defined(CONFIG_CELL_1_TYPE_O2S)
	{
		 ...
		 * so cellSample ~= precision_ppo2 / calCoeff */
		float approx_sample = (float)cell_data.precision_ppo2 /
					      O2S_CAL_DEFAULT;

		new_coeff = o2s_cal_coefficient(approx_sample, target_ppo2);
	}
#endif
		break;
#endif /* CELL_COUNT >= 1 */
...
#if defined(CONFIG_CELL_2_TYPE_ANALOG)
	{
		...
		if (zbus_chan_read(&chan_cell_2, &cell_data, K_MSEC(100)) != 0) {
			result = CAL_RESULT_FAILED;
			break;
		}
		int16_t approx_counts = (int16_t)(cell_data.millivolts /
...
#if defined(CONFIG_CELL_3_TYPE_ANALOG)
	{
		...
		if (zbus_chan_read(&chan_cell_3, &cell_data, K_MSEC(100)) != 0) {
			result = CAL_RESULT_FAILED;
			break;
		}
		int16_t approx_counts = (int16_t)(cell_data.millivolts /
	...
	if (new_coeff < 0.0f) {
		...
		return CAL_RESULT_REJECTED;
	}

	/* Save and verify the coefficient */
	if (cal_save_coefficient(cell_num, new_coeff) != 0) {
		OP_ERROR_DETAIL(OP_ERR_FLASH, cell_num);
		return CAL_RESULT_FAILED;
	}
	...
	if (fabsf(readback - new_coeff) > 1e-5f) {
					 ...
					 CalResponse_t *resp)
{
	...
	if (zbus_chan_read(&chan_cell_1, &ref_data, K_MSEC(100)) == 0) {
		found_ref = true;
	}
#elif CONFIG_CELL_COUNT >= 2 && defined(CONFIG_CELL_2_TYPE_DIVEO2)
	if (zbus_chan_read(&chan_cell_2, &ref_data, K_MSEC(100)) == 0) {
		found_ref = true;
	}
#elif CONFIG_CELL_COUNT >= 3 && defined(CONFIG_CELL_3_TYPE_DIVEO2)
	if (zbus_chan_read(&chan_cell_3, &ref_data, K_MSEC(100)) == 0) {
		found_ref = true;
	}
#endif
	...
	req->fo2 = (FO2_t)roundf((float)ppo2 * (1000.0f / (float)pressure_mbar));
				      ...
				      CalResponse_t *resp)
{
	...
	const uint8_t flush_time_seconds = 25;
	...
	for (uint8_t i = 0; i < flush_time_seconds; i++) {
		...
		(void)sol_o2_flush_fire(1000000U); /* 1 second in microseconds */
#elif defined(CONFIG_HAS_O2_SOLENOID)
		(void)sol_o2_inject_fire(1000000U);
#endif
		k_msleep(1000);
	}
...
static void cal_thread_fn(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		if (zbus_sub_wait_msg(&cal_sub, &chan, &req,
				      K_FOREVER) != 0) {
			continue;
		}
		...
		atomic_clear(&cal_running);
	}
}

K_THREAD_DEFINE(cal_thread, 1024,
