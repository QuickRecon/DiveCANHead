 ...
 * high, recovery briefly pulses the STM32 peripheral-enable bit without
 * unregistering the target. Nine-clock physical bus clear is reserved for a
 * confirmed SCL-high/SDA-low stuck condition; active/toggling traffic is never
 * bit-banged. Takes i2c1_bus_lock() internally.
 *
 ...
 * bus causes (including arbitration loss) as generic -EIO. The policy
 * therefore retries the documented transport errors and relies on
 * physical-line classification before recovery instead of inspecting
