...
- **i2c1 is multi-master on Poseidon.** On `Poseidon_Aren` the transducer ADS1115 shares i2c1 with the Poseidon accessory bus (`poseidon_accessories.c` drives the HUD/battery as master and answers the display as target). An ADS conversion-trigger write racing a Poseidon output frame surfaced as intermittent `-EBUSY` (`OP_ERR_EXT_ADC` detail 0x10). Defence is three layers in `src/i2c_bus_lock.c`:
  1. **Serialise** — both STM32-initiated masters take `i2c1_bus_lock()` (an unconditional `K_MUTEX`) so they never collide with each other. Uncontended (near-free) on variants that don't share the bus.
  2. **Retry with backoff** — the sampler retries the read on the transient `-EBUSY`/`-EAGAIN` that the *external* Poseidon masters can still cause and that the mutex cannot arbitrate. Backoff is exponential with cycle-counter jitter (`tank_adc_backoff()`) so two contending masters don't retry in lockstep; `k_msleep` yields so the competing master and the external display can drain the bus. `send_retry()` in `poseidon_accessories.c` uses the same shape.
  3. **Recover** — a registered i2c *target* keeps the STM32 I2C peripheral permanently enabled (`i2c_stm32_v2.c` skips the between-transfer `LL_I2C_Disable` while a target is attached), so a `BUSY` flag latched by a collision never self-clears and wedges every future read. When retries are exhausted the sampler calls `i2c1_bus_recover()`, which does two things: `i2c_recover_bus()` bit-bangs a STOP over the board's `scl-gpios`/`sda-gpios` (releases a slave physically holding SDA; needs `CONFIG_I2C_STM32_BUS_RECOVERY=y`, set only on `Poseidon_Aren`), then `i2c1_reset_peripheral()` pulses `CR1.PE` to clear a *peripheral*-latched `BUSY` that the bit-bang can't reach (the STM32 only clears `BUSY` on an observed STOP or `PE=0`, and during bit-bang the pins are muxed away from the peripheral). The `PE` pulse is a documented driver-bypass — see COMPROMISE.md #13.
