...
def test_bootloader_reset(divecan_client_fixture: DiveCAN.DiveCAN) -> None:
    """ Test that if we trip into the bootloader, the usual pings will kick us back to normal """
    divecan_client_fixture.send_bootloader()
    time.sleep(5)
    divecan_client_fixture.flush_rx()
    # Make sure we don't hear anything
    with pytest.raises(DiveCAN.DiveCANNoMessageException):
        divecan_client_fixture.listen_for_ppo2()

    # This should kick us out of the bootloader
    divecan_client_fixture.send_id(1)
    time.sleep(5)
    divecan_client_fixture.send_id(1)
    divecan_client_fixture.listen_for_name() # We should get a ping back with no error


def test_bootloader_stuck(divecan_client_fixture: DiveCAN.DiveCAN) -> None:
